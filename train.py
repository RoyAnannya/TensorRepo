...
# Model choices
tf.flags.DEFINE_string('clf', 'cnn', "Type of classifiers. Default: cnn. You have 2 choices: [cnn, clstm]")

# Data parameters
tf.flags.DEFINE_string('data_file', None, 'Data file path')
tf.flags.DEFINE_string('stop_word_file', None, 'Stop word file path')
tf.flags.DEFINE_string('language', 'en', "Language of the data file.")
...
tf.flags.DEFINE_integer('hidden_size', 128, 'Number of hidden units in the LSTM cell. For C-LSTM')
tf.flags.DEFINE_integer('num_layers', 2, 'Number of the LSTM cells. For C-LSTM')
...
data, labels, lengths, vocab_processor = data_utils.load_data(file_path=FLAGS.data_file,
...
vocab_processor.save(os.path.join(data_utils.outdir, 'vocab'))
...
params_file = open(os.path.join(data_utils.outdir, 'params.pkl'), 'wb')
pkl.dump(params, params_file, True)
params_file.close()
print("parameters saved to file \'params.pkl\'")

# Simple Cross validation
print("train_test_split and cross validation set made")
...
print(x_train[:])
# Batch iterator
train_data = data_utils.batch_iter(x_train, y_train, train_lengths, FLAGS.batch_size, FLAGS.num_epochs)
print("=====>>>train_data generator is successfully created with Tensorflow flags")
...
with tf.Graph().as_default():
    with tf.Session() as sess:
        print("................................Inside Tensorflow Activated Session................................")
        if FLAGS.clf == 'cnn':
            classifier = cnn_clf(FLAGS)
        elif FLAGS.clf == 'clstm':
            classifier = clstm_clf(FLAGS)
        else:
            raise ValueError('clf should be one of [cnn, clstm]')
        ...
        # Summaries
        print ("Recording Summaries")
        loss_summary = tf.summary.scalar('Loss', classifier.cost)
        accuracy_summary = tf.summary.scalar('Accuracy', classifier.accuracy)

        # Train summary
        print ("Recording Train Summaries in file")
        train_summary_op = tf.summary.merge_all()
        train_summary_dir = os.path.join(data_utils.outdir, 'summaries', 'train')
        train_summary_writer = tf.summary.FileWriter(train_summary_dir, sess.graph)

        # Validation summary
        print ("Recording Validation Summaries in file")
        valid_summary_op = tf.summary.merge_all()
        valid_summary_dir = os.path.join(data_utils.outdir, 'summaries', 'valid')
        ...
        for train_input in train_data:
            ...
            if current_step % FLAGS.save_every_steps == 0:
                save_path = saver.save(sess, os.path.join(data_utils.outdir, 'model/clf'), current_step)
